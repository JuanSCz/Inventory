...
function fetchProductosPorDeposito(depositoId) {
    const tableBody = document.getElementById('productosTableBody');
    tableBody.innerHTML = '<tr><td colspan="8" class="text-center">Cargando...</td></tr>';

    fetch(`/tableDepositosProductos?depositoId=${depositoId}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(html => {
            ...
            const newTableBody = doc.querySelector('#productosTableBody');
